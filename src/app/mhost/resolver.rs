...
fn filter_by_family(group: NameServerConfigGroup, config: &AppConfig) -> NameServerConfigGroup {
    ...
    let filtered: Vec<_> = group.into_iter().filter(|ns| config.ip_allowed(ns.ip_addr())).collect();
    let after = filtered.len();
    if after < before {
        let family = if config.ipv4_only { "IPv4" } else { "IPv6" };
        debug!("Filtered nameservers by {}: {} -> {} servers", family, before, after,);
    }
...
pub async fn resolve_missing_glue(
    app_config: &AppConfig,
    ns_servers: &mut std::collections::HashMap<String, Vec<std::net::IpAddr>>,
) {
