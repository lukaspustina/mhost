...
impl Dnssec {
    pub async fn init<'a>(app_config: &'a AppConfig, config: &'a DnssecConfig) -> PartialResult<DnssecRun<'a>> {
...
impl<'a> DnssecRun<'a> {
    pub async fn execute(self) -> PartialResult<DnssecOutput<'a>> {
        ...
        info!("Starting DNSSEC trust chain walk for {}", self.domain_name);
...
fn write_level<W: Write>(
    ...
    for (j, key) in level.dnskeys.iter().enumerate() {
        ...
        let tag_str = key.key_tag().map(|t| format!("tag={}", t)).unwrap_or_default();
    ...
    let dnskey_rrsigs: Vec<&crate::resources::rdata::RRSIG> =
        level.rrsigs.iter().filter(|r| r.type_covered() == "DNSKEY").collect();
    ...
    if let Some(ds_records) = next_ds {
        if !ds_records.is_empty() {
            ...
            let child_dnskey_refs: Vec<&crate::resources::rdata::DNSKEY> =
                next_dnskeys.map(|keys| keys.iter().collect()).unwrap_or_default();
...
fn format_finding_inline(finding: &dnssec_validation::Finding) -> String {
    match finding.severity {
        Severity::Ok => format!("  {}", "valid".paint(output_styles::OK)),
        Severity::Warning => format!("  {}", finding.message.paint(output_styles::ATTENTION)),
        Severity::Failed => format!("  {}", finding.message.paint(output_styles::ERROR)),
    }
}
