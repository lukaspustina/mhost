...
impl Verify {
    pub async fn init<'a>(app_config: &'a AppConfig, config: &'a VerifyConfig) -> PartialResult<VerifyCompare<'a>> {
        ...
        let zone_soa = if config.ignore_soa { None } else { zone.soa().cloned() };
        ...
        if zone_records.is_empty() {
            env.console.attention("Zone file contains no verifiable records.");
        }
        ...
        for (name, rtype) in &query_pairs {
            names_to_types.entry(name.clone()).or_default().insert(*rtype);
        }
...
impl<'a> VerifyCompare<'a> {
    pub fn compare(self) -> VerifyOutput<'a> {
                    ...
                    for expected_record in expected_records {
                        if actual_records.contains(expected_record) {
                            ...
                            if self.env.mod_config.strict {
                                if let Some(actual_record) = actual_records.iter().find(|r| *r == expected_record) {
        ...
        debug!(
            "Verify results: {} matched, {} missing, {} extra, {} TTL drifts, {} wildcards skipped, SOA check: {:?}",
            results.matches.len(),
            results.missing.len(),
            results.extra.len(),
        ...
        VerifyOutput { env: self.env, results }
    }
}
...
impl VerifyResults {
    pub fn has_issues(&self) -> bool {
        !self.missing.is_empty()
            ...
            || self.soa_check.as_ref().is_some_and(|check| !check.match_)
    }
}
...
mod tests {
    ...
    fn make_a_record(name: &str, ip: Ipv4Addr, ttl: u32) -> Record {
        Record::new_for_test(Name::from_utf8(name).unwrap(), RecordType::A, ttl, RData::A(ip))
    }
    ...
    fn verify_results_extra_not_an_issue() {
        ...
        assert!(
            !results.has_issues(),
            "extra records alone are informational, not issues"
        );
    }
