...
use crate::app::common::discover::ct_logs;
use crate::app::common::discover::permutation;
use crate::app::common::discover::srv_probing;
use crate::app::common::discover::txt_mining;
use crate::app::common::discover::wordlist::Wordlist;
...
impl<'a> WildcardCheck<'a> {
    ...
    pub fn rnd_names(number: usize, len: usize) -> Vec<String> {
        crate::app::common::discover::rnd_names(number, len)
    }
}
...
impl<'a> NsecWalking<'a> {
    pub async fn nsec_walking(self) -> PartialResult<WordlistLookups<'a>> {
        ...
        if let Ok(query) = MultiQuery::multi_record(self.domain_name.clone(), vec![RecordType::NSEC, RecordType::NSEC3])
        {
            match time(self.app_resolver.lookup(query)).await {
                Ok((nsec_lookups, _)) => {
...
// ---------------------------------------------------------------------------
// Re-export for backward compatibility (used by mdive)
pub use crate::app::common::discover::filter_wildcard_responses;
