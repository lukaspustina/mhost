...
impl Trace {
    pub async fn init<'a>(app_config: &'a AppConfig, config: &'a TraceConfig) -> PartialResult<TraceRun<'a>> {
...
impl<'a> TraceRun<'a> {
    pub async fn execute(self) -> PartialResult<TraceOutput<'a>> {
        ...
        let hops = self
            .walk_delegation(&hickory_name, hickory_record_type, record_type, max_hops, timeout)
            .await;
    ...
    async fn walk_delegation(
        ...
        let mut current_servers =
            delegation::root_server_addrs(self.env.app_config.ipv4_only, self.env.app_config.ipv6_only);
        ...
        for level in 0..max_hops {
            ...
            let server_names: HashMap<SocketAddr, Option<String>> = current_servers.iter().cloned().collect();
            ...
            let (server_results, next_servers, is_final) = process_hop_results(&results, &server_names, record_type);
...
fn write_hop<W: Write>(
    ...
    if hop.is_final {
        ...
        if hop.referral_groups.len() <= 1 {
            ...
            if let Some(group) = hop.referral_groups.first() {
                ...
                writeln!(writer, " {} NS: {}", output_styles::info_prefix(), ns_list,)?;
            }
        } else {
            // Divergence
            writeln!(
    ...
    if show_all_servers {
        for sr in &hop.server_results {
            let name_str = sr.server_name.as_deref().unwrap_or("unknown");
        ...
        for err_result in errors {
            if let ServerOutcome::Error(ref msg) = err_result.outcome {
                writeln!(
                    ...
                    err_result.server_name.as_deref().unwrap_or("unknown"),
...
mod tests {
    ...
    fn compute_referral_groups_single_group() {
        let results = vec![
            ServerResult {
                ...
                outcome: ServerOutcome::Referral,
                answer_records: Vec::new(),
                referral_ns: vec!["a.gtld-servers.net.".to_string(), "b.gtld-servers.net.".to_string()],
    ...
    fn make_server_names(addrs: &[SocketAddr], names: &[Option<String>]) -> HashMap<SocketAddr, Option<String>> {
    ...
    fn process_hop_referral_extracts_ns_and_glue() {
        ...
        let (server_results, next_servers, is_final) = process_hop_results(&results, &names, RecordType::A);

        assert!(!is_final);
        assert_eq!(server_results.len(), 1);
        assert!(matches!(server_results[0].outcome, ServerOutcome::Referral));
    ...
    fn process_hop_authoritative_answer() {
        ...
        assert_eq!(server_results[0].server_name, Some("ns1.example.com.".to_string()));
    ...
    fn process_hop_authoritative_nxdomain() {
        ...
        let (server_results, _, is_final) = process_hop_results(&results, &names, RecordType::A);
    ...
    fn process_hop_error_result() {
        ...
        assert!(!is_final);
        assert_eq!(server_results.len(), 1);
        assert!(matches!(server_results[0].outcome, ServerOutcome::Error(_)));
    ...
    fn process_hop_referral_without_glue() {
        ...
        let response = make_referral_response("example.com.", &["ns1.otherdomain.net."], &[]);
