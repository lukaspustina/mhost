...
impl<'a> OpenResolver<'a> {
    ...
    async fn do_open_resolver(&self) -> PartialResult<Vec<CheckResult>> {
        ...
        let lookups: Lookups = intermediate_lookups!(
            self,
            query,
    ...
    async fn check_open_resolver(ns_ips: &[std::net::IpAddr], app_config: &crate::app::AppConfig) -> Vec<CheckResult> {
        ...
        for ip in ns_ips.iter().take(3) {
            ...
            match AppResolver::from_configs(vec![resolver_config], app_config).await {
                    ...
                    if let Ok(query) = MultiQuery::multi_record(probe_name.clone(), vec![RecordType::A]) {
                        match tokio::time::timeout(Duration::from_secs(5), probe_resolver.lookup(query)).await {
                            Ok(Ok(probe_lookups)) if probe_lookups.has_records() => {
                                info!(
                                    "Open resolver detected at {} - answered recursive query for external domain",
                                    ip
                                );
                                open_ips.push(*ip);
                            }
                            Ok(Ok(_)) => {
                                debug!(
                                    "No records returned from {} for external query (properly configured)",
                                    ip
                                );
                            }
                            Ok(Err(e)) => {
                ...
                Err(e) => {
                    debug!(
                        "Failed to create resolver for open resolver probe against {}: {}",
                        ip, e
                    );
                }
            }
