...
impl<'a> Delegation<'a> {
    ...
    async fn do_delegation(&self) -> PartialResult<Vec<CheckResult>> {
        ...
        let parent_zone_lookups: Lookups = intermediate_lookups!(
            self,
            parent_ns_query,
            "Looking up parent zone NS records for delegation check."
        );
        ...
        let ns_lookups: Lookups = intermediate_lookups!(
            self,
            resolve_query,
        ...
        for ip in parent_ns_ips.iter().take(3) {
            info!(
                "Querying parent NS {} for delegation records of {}",
                ip, self.domain_name
            );
