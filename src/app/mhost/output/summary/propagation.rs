...
impl SummaryFormatter for PropagationResults {
    fn output<W: Write>(&self, writer: &mut W, opts: &SummaryOptions) -> Result<()> {
        ...
        if !self.authoritative_ns.is_empty() {
            writeln!(writer, "{} Authoritative Nameservers:", output_styles::info_prefix())?;
            ...
            if let Some(ref soa) = self.soa_details {
                ...
                writeln!(writer, "  {} SOA serial: {}", output_styles::info_prefix(), serial)?;
            }

            // List each NS compactly
            for ns in &self.authoritative_ns {
                let serial_str = ns.serial.map(|s| format!(" \u{2014} serial {}", s)).unwrap_or_default();
                let error_str = ns
                    .error
                    .as_ref()
            ...
            // Sync status
            let serials: Vec<u32> = self.authoritative_ns.iter().filter_map(|ns| ns.serial).collect();
        ...
        writeln!(writer, "{} Recursive Resolvers:", output_styles::info_prefix())?;

        if self.resolver_groups.is_empty() {
            writeln!(writer, " {} No responses received.", output_styles::info_prefix())?;
        } else {
        ...
        if !self.unreachable_servers.is_empty() {
            writeln!(
                ...
                if self.unreachable_servers.len() == 1 { "" } else { "s" },
            )?;
        ...
        if self.authoritative_serial.is_some() {
            ...
            if self.is_fully_propagated() {
                writeln!(writer, "{} Propagation: 100% complete.", output_styles::ok_prefix())?;
            } else {
                writeln!(
                    writer,
...
fn write_records<W: Write>(writer: &mut W, records: &[Record], opts: &SummaryOptions) -> crate::Result<()> {
    if records.is_empty() {
        return Ok(());
    }
    ...
    for record in sorted {
        writeln!(tw, " {} {}", output_styles::itemization_prefix(), record.render(opts))?;
    }
    let buf = tw.into_inner().map_err(|_| crate::Error::InternalError {
