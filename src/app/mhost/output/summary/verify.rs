...
impl SummaryFormatter for VerifyResults {
    fn output<W: Write>(&self, writer: &mut W, opts: &SummaryOptions) -> Result<()> {
        writeln!(writer, "DNS Verification for {} (from {})", self.origin, self.zone_file)?;
        writeln!(writer)?;

        let soa_ok = self.soa_check.as_ref().is_none_or(|check| check.match_);

        if self.missing.is_empty() && self.extra.is_empty() && self.ttl_drifts.is_empty() && soa_ok {
            writeln!(
                writer,
