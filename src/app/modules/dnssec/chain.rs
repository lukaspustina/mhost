    ...
    F: FnMut(&[DelegationLevel]),
{
    ...
    let mut current_servers =
        delegation::root_server_addrs(app_config.ipv4_only, app_config.ipv6_only);
    ...
    for (i, zone) in zones.iter().enumerate().take(walk_len) {
        ...
        findings.extend(dnssec_validation::validate_rrsig_dnskey_binding(&rrsig_refs, &dnskey_refs));
        ...
        if dnskeys.is_empty() && rrsigs.is_empty() && i > 0 {
            findings.push(Finding::warning(format!("Zone {} has no DNSKEY records: not DNSSEC-signed", zone)));
        }
