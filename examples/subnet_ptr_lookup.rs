...
async fn main() {
    ...
    for l in lookups {
        if let Some(response) = l.result().response() {
            let names = l.ptr();
            if !names.is_empty() {
                let names = names.into_iter().map(ToString::to_string).collect::<Vec<_>>().join(", ");
