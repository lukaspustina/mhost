...
struct VerifyResults {
    matches: Vec<RecordMatch>,
    missing: Vec<Record>,         // expected but not in live DNS
    mismatches: Vec<RecordMismatch>,  // expected vs. actual differ
    extra: Vec<Record>,           // in live DNS but not in zone file
    ttl_drifts: Vec<TtlDrift>,   // only populated in strict mode
    errors: Vec<QueryError>,      // lookups that failed
}
```
...
- `--only-type <TYPE>` — Only verify records of this type (repeatable)
- `--ignore-type <TYPE>` — Skip records of this type (repeatable)
...
- **SOA**: Serial numbers change on every zone update; verifying rdata byte-for-byte is usually not useful. SOA serial comparison could be a separate check ("is the live serial >= my zone file serial").
...
- `--only-type` / `--ignore-type` filtering
- `--server` override for authoritative nameserver targeting
- `--ignore-extra` to suppress extra record reporting
- Integration with existing `check` lints on the parsed zone
- SOA serial comparison (separate from record-by-record verification)
